...
if __name__ == '__main__':
    # 两个静态站定位误差序列对比绘图
    # 输入： 两个对比的pos文件 一个snx文件 一个图片保存路径
    pos_type = 2  # 设置pos文件的类型 [1:RTKLIB 2:GAMP 3:Net_Diff 4:IE]
    pos_file1 = r'C:\Users\LZ\Desktop\arlz2\result\SPP_KLO\harb2740.21o.pos'
    pos_file2 = r'C:\Users\LZ\Desktop\arlz2\result\SPP_KLO_BDS2\harb2740.21o.pos'
    ...
    site = getsite(pos_file1)
    xyz = getcrd(site, snxfile)        # 根据测站名在snx文件中搜索测站精确坐标XYZ
    # xyz = [5084657.60627794, 2670325.43685606, -2768480.87040129]  # change the coordinate [X, Y, Z]
    list1 = readpos(pos_file1, pos_type)    # 读取第一个POS文件
    list2 = readpos(pos_file2, pos_type)    # 读取第二个POS文件
    ...
    for i in range(len(list2)):
        if list2[i] != []:
            enu2.append(xyz2enu(list2[i][0:3], xyz))
